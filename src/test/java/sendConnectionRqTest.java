    ...
    public void sendConnectionRequest()  {
        try {
            ...
            try {
                ...
                List<Contact> contacts = ExcelHandler.getContactsFromExcel(limit);
                for (Contact contact : contacts) {
                    ...
                    profilePage.sendWithoutMessage(); //send without invite note
                    ...
                    getTest().info("Visited profile of " + contact.getFirstName() + "\t" + contact.getLastName());
                    getTest().pass("Connection request from "+sharedAccountKey+" sent to: " + contact.getFirstName());

                }

            } catch (Exception e) {
                getTest().fail("Failed to send request to profile.Error: " + e.getMessage());

            }



            getTest().info("Total profiles visited for " +sharedAccountKey+" are "+ profilesVisited);
            getTest().pass("Completed sending connection requests for account "+sharedAccountKey);

        } catch (Exception e) {
            getTest().fail("Failed to complete connection requests for account: " + ". Error: " + e.getMessage());
